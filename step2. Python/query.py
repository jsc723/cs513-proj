...
def main():
    ...
    review_count_df = ps.sqldf('''SELECT taster_name, taster_twitter_handle, count(*) as num_of_reviews FROM df GROUP BY taster_name ORDER BY num_of_reviews DESC''')
    print(review_count_df)
    most_significant_reviewer = review_count_df['taster_name'][0]
    twitter_handle = review_count_df['taster_twitter_handle'][0]
    num_of_reviews = review_count_df['num_of_reviews'][0]
    print(f'most_significant_reviewer: "{most_significant_reviewer}": {twitter_handle}, num_of_reviews: {num_of_reviews}')
